...
class App {
    ...
    public function __construct() {
        ...
        if(isset($this->url[0])) {
            if(file_exists(__DIR__ . '/../controllers/'. ucfirst($this->url[0]).'Controller'.'.php')) {
                // Set the controller
        ...
        require_once(__DIR__ . '/../controllers/'. ucfirst($this->controller).'Controller'.'.php');

        /**
         * The namespace\class must be defined in a string as it can't be called shorted using new namespace\$var
         */
        $class = 'Vonzo\Controllers\\'.ucfirst($this->controller).'Controller';
