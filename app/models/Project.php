...
class Project extends Model {
	...
	function getProjectProfileById($xProject, $awardModel, $awardShowModel, $awardShowYearModel, $personModel, $projectModel, $vendorModel, $studioModel) {
		// START nominations
		$query = 'SELECT * FROM `z_nominations` WHERE FIND_IN_SET('.$xProject['id'].', project_id_array) > 0';
		...
		if ($nomResult > 0) {
			...
			while($nomRow = mysqli_fetch_array($nomRows)) {
				...
				if ($xxx == 0) {
					$lastShow = $xShow['full_name'];
					$nominations .= '<li style="list-style-type:none;">'.$nomRow['show_year'].' '.$xShow['full_name'].'</li>';
					$lastShow = $xShow['full_name'];
				}
				$xxx++;

				if ($lastShow != $xShow['full_name']) {
					$nominations .= '<li style="list-style-type:none;padding-top:8px;">'.$nomRow['show_year'].' '.$xShow['full_name'].'</li>';
					$lastShow = $xShow['full_name'];
				}

				$personIdArray = explode(",",$nomRow['person_id_array']);
				// $projectIdArray = explode(",",$nomRow['project_id_array']);
				// $vendorIdArray = explode(",",$nomRow['vendor_id_array']);
				// $studioIdArray = explode(",",$nomRow['studio_id_array']);
				...
				$nominations .= '<li>
													<a href="'.URL_PATH.'/admin/shows/'.$nomRow['show_id'].'/year/'.$xAwardShowYear['id'].'/category/'.$nomRow['award_id'].'/nomination/'.$nomRow['id'].'">'.$xAward['name'].'</a> | '.$person.' <span class="bold" style="text-decoration:none;">'.$nomRow['status'].'</span>
												 </li>';
			}
		}
		// END NOMINATIONS
		...
		if ($xProject['person_id_array'] != "") {
			$projectIdArray = explode(",", $xProject['person_id_array']);
		...
		$xProject['project_id_array'] = $xProject['id'];
		if ($xProject['project_id_array'] != "") {
			$projectIdArray = explode(",", $xProject['project_id_array']);
			for ($i = 0; $i < sizeof($projectIdArray); $i++) {
		...
		if ($xProject['studio_id_array'] != "") {
			$studioIdArray = explode(",", $xProject['studio_id_array']);
		...
		$page = '<h6>Project<h6>
		<h4 class="page-title-bold mb-3 mt-0">'.$xProject['title_name'].'</h4>';
		...
		$page .= '<div class="row">';
			...
			$page .= '<div class="col-md-4">';
				$page .= "<div style='padding: 1em; background-color: rgba(0,100,200,0.3);'>
										<p>
											<b>Title Name</b><br>
											<input type=text id='p_title_name' size=30 value='".$xProject['title_name']."' disabled>
										</p>

									<input type='hidden' id='project_id' value='".$xProject['id']."'>

									<div class='custom-button edit_project_submit' style='margin-left:0'>Update</div>
								</div>";

								$page .= '<h6 class="mt-3">Credits<h6><BR><BR>';
								...
								// 				</thead>
								// 				<tbody>';
								// 					while ($xTv = mysqli_fetch_array($tv)) {
								// 						$page .= '
								// 						<tr>
								// 							<td>'.$xTv['season'].'</td>
								// 							<td>'.$xTv['episode'].'</td>
								// 							<td>'.$xTv['episode_title'].'</td>
								// 						</tr>
								// 						';
								// 					}
								// 				$page .=
			...
			$page .= '<div class="col-md-8" style="padding-left:0;padding-right:0;margin-right:right:0;margin-left:0;">';
				...
				$page .= '<fieldset style="margin-top:0;margin-left:10px;margin-right:10px;">
										...
										<input type="hidden" id="media_type" value="Project">
										<input type="hidden" id="media_type_id" value="'.$xProject['id'].'">
		...
		// $page .= '<div class="row">';
				// $page .= '<div class="col-md-12" style="padding-left:0;padding-right:0;margin-right:right:0;margin-left:0;">';
				// 	$page .= '<fieldset style="margin-left:10px;margin-right:10px;"><legend>Media</legend>'.$artifactImages.'</fieldset>';
				// 	$page .= '<fieldset style="margin-top:10px;margin-left:10px;margin-right:10px;"><legend>Documents</legend>'.$artifactDocuments.'</fieldset>';
				//
				// 	$page .= '<fieldset style="margin-top:0;margin-left:10px;margin-right:10px;">
	...
	function listProjects($search, $awardModel, $awardShowModel) {
		...
		$reportNav = "";
		$actual_link = URL_PATH."/a/index.php?view=projects";
		// if ($page > 0) {
	  //   $last = $page - 2;
	  ...
	  //  }
		...
		if ($result > 0) {
			while ($row = mysqli_fetch_array($rows)) {
				if (trim($row['title_name']) != "") {
					...
					$project_media_rows = $this->getMeidaByProjectId($row['id']);
					$project_media_return = "";
					if ($project_media_rows) {
						while ($xProjectMedia = mysqli_fetch_array($project_media_rows)) {
							 $url = "http://www.awardsawardsawards.com/assets/artifact/".$xProjectMedia['file_path'];
								switch ($xProjectMedia['type'])
								{
									case "Image":
										$project_media_return .= "<a href=?view=artifacts&id=".$xProjectMedia['id']."><img src=$url height=60></a>";
										break;
									case "Document":
										$project_media_return .= "<a href=$url target=_blank>View</a>";
										break;
								}
						}
					}

					// $queryContent = urlencode(trim($row['title_name']));
					$reportRows .= '<tr bgcolor=white>
														<td>Project</td>
														<td>
															<a href="?view=projects&id='.$row['id'].'">'.$row['title_name'].'</a>
															<br>'.$nominations.'
														</td>
														<td></td>
														<td></td>
														<td>'.$project_media_return.'</td>
														<td></td>
														<td></td>
													</tr>';
				}
		...
		$form = '<h4 class="page-title-bold mb-3 mt-0">Projects ('.$totalCount['total'].')</h2><p><form method=post><input type=hidden name=action value=searchType><input type=hidden name=type value=Project><input type=text name=searchKey><input type=submit name=submit value=search><p>';
		$report .= $form;
		if ($q == "") {
	...
	function getProjectTvsByProjectId($project_id) {
		$result = mysqli_query($this->db, 'SELECT * FROM `z_project_tv` WHERE `project_id_array` = '.$project_id);
	...
	function getProjectTvByProjectId($project_id) {
		$result = mysqli_fetch_array(mysqli_query($this->db, 'SELECT * FROM `z_project_tv` WHERE `project_id_array` = '.$project_id));
		// array_walk_recursive($result, '_clean');
		if($result) {
			return $result;
		}
	...
	public function doesProjectExistById($id) {
		if ($id != "") {
			$result = mysqli_fetch_array(mysqli_query($this->db, 'SELECT * FROM `z_project` WHERE `id` = '.$id));
			// array_walk_recursive($result, '_clean');
			if($result) {
				return $result;
			}
	...
	public function getMeidaByProjectId($project_id) {
		$result = mysqli_query($this->db, 'SELECT * FROM `z_media` WHERE `project_id` = "'.$project_id.'"');
		// array_walk_recursive($result, '_clean');
		if($result) {
			return $result;
		}
