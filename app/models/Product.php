...
class Product extends Model {
	...
	public function insertNewProduct($product_name) {
		$query = $this->db->prepare("INSERT INTO `z_product` (`company_name`) VALUES (?);");
		$query->bind_param('s', $product_name);
		if (!$query->execute()) {
		 error_log("insertNewProduct Execute failed: (" . $query->errno . ") " . $query->error);
		}
		$new_product_id = $query->insert_id;
		$query->close();
		return $new_product_id;
	}
	public function doesProductNameExist($product_name) {
		if ($product_name != "") {
			$result = mysqli_query($this->db, 'SELECT * FROM `z_product` WHERE `company_name` LIKE "'.$product_name.'"');
	...
	function getProductProfileById($xProduct, $awardModel, $awardShowModel, $awardShowYearModel) {
		// START nominations
		$query = 'SELECT * FROM `z_nominations` WHERE FIND_IN_SET('.$xProduct['id'].', product_id_array) > 0';
		...
		$query = 'SELECT * FROM `z_media` WHERE `product_id` = ' . $xProduct['id'];
		...
		// if ($xProduct['person_id_array'] != "") {
		// 	$projectIdArray = explode(",", $xProduct['person_id_array']);
		...
		// }
		//
		// if ($xProduct['project_id_array'] != "") {
		// 	$projectIdArray = explode(",", $xProduct['project_id_array']);
		// 	for ($i = 0; $i < sizeof($projectIdArray); $i++) {
		...
		// }
		//
		// if ($xProduct['studio_id_array'] != "") {
		// 	$studioIdArray = explode(",", $xProduct['studio_id_array']);
		...
		$page = '<h6>Product<h6>
		<h4 class="page-title-bold mb-3 mt-0">'.$xProduct['company_name'].'</h4>';
		...
		$page .= '<div class="row">';
			...
			$page .= '<div class="col-md-4">';
				$page .= "<div style='padding: 1em; background-color: rgba(0,100,200,0.3);'>
										<p>
											<b>Product Name</b><br>
											<input type=text id='p_company_name' size=30 value='".$xProduct['company_name']."' disabled>
										</p>

									<input type='hidden' id='project_id' value='".$xProduct['id']."'>
			...
			$page .= '<div class="col-md-4">';
				...
				$page .= '<fieldset style="margin-top:10px;margin-left:10px;margin-right:10px;">
										...
										<input type="hidden" id="media_type" value="Product">
										<input type="hidden" id="media_type_id" value="'.$xProduct['id'].'">
	...
	function listProducts($search, $awardModel, $awardShowModel) {
		...
		$reportNav = "";
		$actual_link = URL_PATH."/a/index.php?view=products";
		// if ($page > 0) {
	  //   $last = $page - 2;
	  ...
	  //  }
		...
		if ($result > 0) {
			while ($row = mysqli_fetch_array($rows)) {
				if (trim($row['company_name']) != "") {
					...
					$product_media_rows = $this->getMeidaByProductId($row['id']);
					$product_media_return = "";
					if ($product_media_rows) {
						while ($xProductMedia = mysqli_fetch_array($product_media_rows)) {
							 $url = "http://www.awardsawardsawards.com/assets/artifact/".$xProductMedia['file_path'];
								switch ($xProductMedia['type'])
								{
									case "Image":
										$product_media_return .= "<a href=?view=artifacts&id=".$xProductMedia['id']."><img src=$url height=60></a>";
										break;
									case "Document":
										$product_media_return .= "<a href=$url target=_blank>View</a>";
										break;
								}
						}
					}
					...
					$reportRows .= '<tr bgcolor=white>
														<td>Product</td>
														<td>
															<a href="?view=products&id='.$row['id'].'">'.$row['company_name'].'</a>
															<br>'.$nominations.'
														</td>
														<td></td>
														<td></td>
														<td>'.$product_media_return.'</td>
														<td></td>
														<td></td>
													</tr>';
				}
		...
		$form = '<h4 class="page-title-bold mb-3 mt-0">Products ('.$totalCount['total'].')</h2><p><form method=post><input type=hidden name=action value=searchType><input type=hidden name=type value=Product><input type=text name=searchKey><input type=submit name=submit value=search><p>';
		$report .= $form;
		if ($q == "") {
	...
	public function doesProductExistById($id) {
		if ($id != "") {
			$result = mysqli_fetch_array(mysqli_query($this->db, 'SELECT * FROM `z_product` WHERE `id` = '.$id)) or die(error_log(print_r(mysqli_error($this->db), true)));
			// array_walk_recursive($result, '_clean');
			if($result) {
				return $result;
			}
	...
	public function getMeidaByProductId($product_id) {
	  $result = mysqli_query($this->db, 'SELECT * FROM `z_media` WHERE `product_id` = "'.$product_id.'"');
	  // array_walk_recursive($result, '_clean');
	  if($result) {
	    return $result;
	  }
